...
export function renderRoadmapMatrix(model, options = {}) {
  ...
  const rows = model.streams.map(stream => `<tr><th scope="row">${escapeHtml(stream.name)}</th>${model.horizons.map(horizon => `<td><div class="rm-matrix-cell">${model.placements.filter(placement => placement.streamId === stream.id && placement.horizonId === horizon.id).map(placement => {
    ...
    return `<a class="rm-matrix-pill rm-theme-${placement.themeId}" href="#${item.id}" data-roadmap-item="${item.id}"${completion.attributes} data-throughlines="${escapeHtml((item.throughlineIds || [placement.themeId]).join(' '))}" data-search-text="${escapeHtml([item.title, completion.statusText, item.statusLabel, item.preview].filter(Boolean).join(' '))}" title="${escapeHtml(item.preview || item.title)}"><span>${escapeHtml(placement.label || item.title)}</span>${status ? `<small>${status}</small>` : ''}</a>`;
